...
def _get_kv_map(s3Path):
    ...
    # response = textract_client.analyze_document(
    #     Document={'S3Object': {'Bucket': KYCDOCUMENTSBUCKET_NAME, 'Name': s3Path}},
    #     FeatureTypes=["QUERIES"],  # Adapter uses QUERIES, not FORMS or TABLES
    #     QueriesConfig={
    #         'Queries': [
...
def extract(s3Path: str):
    ...
    try:
        ...
        # with adapter
        # key_map, value_map, block_map, blocks, response = _get_kv_map(s3Path)
        ...
        # with adapter
        # extractedData = dict(form = query_answers, phrases = text_phrases)
